...
class ButtonHandler {
        ...
        try {
            ...
            switch (action) {
                ...
                case 'lb':
                    ...
                    break;
                case 'illegal':
                    await this.handleIllegalButton(client, interaction, params);
                    break;
                default:
    ...
    async handleIllegalButton(client, interaction, params) {
        const [action, serviceKey, userId] = params;
        ...
        try {
            ...
            if (action === 'cancel') {
                const embed = new MessageEmbed()
                    ...
                    .setDescription('Yasadışı hizmet satın alma işlemi iptal edildi.')
                    .setTimestamp();
            ...
            if (action === 'confirm') {
                ...
                if (!service) {
                    return interaction.reply({
                        content: '❌ Geçersiz hizmet!',
                        ephemeral: true
                    });
                }

                const userData = economy.getUserData(interaction.guild.id, userId);
                ...
                if (service.needsHouse) {
                    const hasHouse = userData.inventory && Object.keys(userData.inventory).some(item => {
                        const shopData = economy.getShopItems(interaction.guild.id);
                    ...
                    if (!hasHouse) {
                        return interaction.reply({
                            content: '❌ Bu hizmet için bir evin olması gerekiyor!',
                            ephemeral: true
                        });
                    }
                }
                ...
                // Para çek
                userData.cash -= service.price;

                // Bonus varsa ekle
                if (service.bonus) {
                    userData.cash += service.bonus;
                }

                // Envanterine hizmeti ekle
                if (!userData.inventory) {
                    userData.inventory = {};
                }
                
                const serviceName = `illegal_${serviceKey}`;
                if (!userData.inventory[serviceName]) {
                    userData.inventory[serviceName] = { count: 0, emoji: service.emoji, category: 'illegal' };
                }
                userData.inventory[serviceName].count += 1;

                economy.setUserData(interaction.guild.id, userId, userData);

                // Success embed
                const embed = new MessageEmbed()
                    .setColor('#8B0000')
                    .setTitle('✅ Yasadışı Hizmet Satın Alındı!')
                    .setDescription(`**${service.name}** başarıyla satın alındı!\n\n${service.response}`)
                    .addField('💰 Ödenen Miktar', economy.formatAmount(service.price), true)
                    .addField('💳 Yeni Bakiye', economy.formatAmount(userData.cash), true);

                if (service.bonus) {
                    embed.addField('🎁 Bonus Kazanç', economy.formatAmount(service.bonus), true);
                }

                embed.setThumbnail(interaction.user.displayAvatarURL({ dynamic: true }))
                    .setTimestamp()
                    .setFooter({ text: 'Yasadışı hizmetler gizli tutulmalıdır!' });

                // Log the transaction
                await logger.logTransaction(client, interaction.guild.id, {
                    type: 'illegal_service',
                    user: userId,
                    service: serviceKey,
                    amount: service.price,
                    bonus: service.bonus || 0
                });
            ...
            console.error('Illegal button handler error:', error);
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
