...
class ButtonHandler {
    ...
    async handleRoleButton(client, interaction, params) {
        try {
            ...
            const [action, ...roleParams] = params;
            ...
            switch (action) {
                ...
                default:
                    await interaction.reply({ 
                        content: '❌ Bilinmeyen rol işlemi!', 
                        ephemeral: true 
                    });
            }

        } catch (error) {
            console.error('Rol butonu hatası:', error);
            if (!interaction.replied) {
                await interaction.reply({ 
                    content: '❌ Rol ayarlarında bir hata oluştu!', 
                    ephemeral: true 
                });
            }
        }
    ...
    async handleRoleSetup(client, interaction, params) {
        const [roleType] = params;
        
        // Role Select Menu oluştur
        const roleSelectEmbed = new EmbedBuilder()
            .setColor(config.colors.primary)
            .setTitle(`${this.getRoleEmoji(roleType)} ${this.getRoleName(roleType)} Seçimi`)
            .setDescription(`**${this.getRoleName(roleType)}** için hangi rolü seçmek istiyorsunuz?\n\nAşağıdaki menüden rolü seçin:`)
            .setTimestamp();

        // Sunucudaki rolleri al (bot rolü hariç)
        ...
        if (roles.length === 0) {
            return interaction.reply({
                content: '❌ Bu sunucuda ayarlanabilir rol bulunamadı!',
                ephemeral: true
            });
        }
        ...
        const selectMenu = new ActionRowBuilder()
            .addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId(`role_select_${roleType}`)
    ...
    async handleRoleBackButton(client, interaction) {
        // Ana rol menüsüne dön
        await this.showRoleSetupMenu(interaction);
    }

    async handleRoleResetButton(client, interaction) {
        try {
            permissions.resetRoles(interaction.guild.id);
            
            const resetEmbed = new EmbedBuilder()
                .setColor(config.colors.success)
                .setTitle(`✅ Roller Sıfırlandı`)
            ...
            await interaction.reply({ 
                content: '❌ Roller sıfırlanırken bir hata oluştu!', 
                ephemeral: true 
            });
        }
    }

    async showRoleSetupMenu(interaction) {
        const setupEmbed = new EmbedBuilder()
            .setColor(config.colors.primary)
            .setTitle(`🔧 Rol Ayarlama Sistemi`)
        ...
        // Dördüncü satır - yardımcı butonlar
